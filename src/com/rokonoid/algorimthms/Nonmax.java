	...
	public double[] process() {
		...
		for (int x = 1; x < width - 1; x++) {
			...
			for (int y = 1; y < height - 1; y++) {
				...
				if (Math.abs(diffx[x + (y * width)]) > Math
					.abs(diffy[x + (y * width)]))
				{
					a1 = mag[(x + dx) + ((y) * width)];
					...
					point =
						mag[x + (y * width)] * Math.abs(diffx[x + (y * width)]);
					if (point >= A && point > B) {
				...
				else
				{
					...
					point =
						mag[x + (y * width)] * Math.abs(diffy[x + (y * width)]);
					if (point >= A && point > B) {
